...
{
    FILE *f;
    //char openFile[10] = "file open: ";
    //strcat(openFile,fname);
    //recordLog(1,"client20",openFile,"");
...
static gboolean save_file(char *fname)
{
	...
	if(resp == GTK_RESPONSE_OK) {
	    ...
	    gtk_widget_destroy(dialog);
	    char openFile[50] = "new file created : ";    //TODO : get file name and directory separately
	    strcat(openFile,fname);
	    recordLog(1,"client0",openFile,"this was triggered because a new file was created");
	} else {
	    gtk_widget_destroy(dialog);
	    return FALSE;
	}
    }else{
        char openFile[50] = "file modified : ";    //TODO : get file name and directory separately
	    strcat(openFile,fname);
	    recordLog(1,"client0",openFile,"this was triggered because file was modified");
    }
	...
	gtk_text_buffer_set_modified(buf, FALSE);
	if(fname != filename) {
	    gchar *wt = g_strdup_printf("File open : (%s)", fname);
	    char openFile[50] = "file saved : ";    //TODO : get file name and directory separately
	    strcat(openFile,fname);
	    recordLog(1,"client0",openFile,"this was triggered because a new file was saved in the system");
	    g_free(filename);
	    filename = fname;
	    gtk_window_set_title(GTK_WINDOW(window), wt);
	    g_free(wt);
...
static gboolean save_if_modified(void)
{
    ...
    gtk_widget_destroy(dialog);
    switch(resp) {
    case GTK_RESPONSE_YES:  //recordLog(11,"client0",filename,"this log record was created when file is modified");
